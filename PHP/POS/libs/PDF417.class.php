...
class PDF417 {
	...
	/**
	 * Stop pattern for truncated mode.
	 * @protected
	 */
	protected $Tstop_pattern = '100000000000000000';
	/**
	 ...
	 * @public
	 */
	public function __construct($code, $ecl=-1, $cols=0, $rows=0, $isTruncated=false, $macro=array()) {
		$barcode_array = array();
		...
		$aspectratio = (pow(69+(34*$cols), 2) - 4761) / 68 / ROWHEIGHT / $nce;
		// adjust rows
		$rows = max($rows,ceil($nce / $cols));
		$size = ($cols * $rows);
		/*
		while($size < $errsize) {
			$rows++;
		...
		*/
		...
		$pTstop = $this->Tstop_pattern.str_repeat('0', QUIETH);
		$barcode_array['num_rows'] = ($rows * ROWHEIGHT) + (2 * QUIETV);
		$barcode_array['num_cols'] = (($cols + (!$isTruncated?2:0)) * 17) + 35 + (2 * QUIETH);

...
class Datamatrix {
	...
	protected $symbattr = [
	  "S"=>[
		...
		/*
		// non viene considerato che [14] vale 0x9b negli ultimi due cicli del getErrorCorrection, da specifica
	  ...
	  "R"=>[
		...
		[0x00c,0x024,0x00a,0x020,0x00c,0x012,0x00a,0x010,0x001,0x002,0x002,0x016,0x012,0x001,0x016,0x012], // 12x36
...
// -----------------------------------------------------------------------------
	 ...
	 * @public
	 */
	public function __construct($code,$shape='S',$cols=0,$rows=0) {
		// $shape is 'R' for rectangular or 'S' for square
		$this->shape=$shape;
		$this->cols=$cols;
		$this->rows=$rows;
		$barcode_array = [];
		
		if ((is_null($code)) OR ($code == '\0') OR ($code == '')) return false;
		// get data codewords
		$cw = $this->getHighLevelEncoding($code);
		// number of data codewords
		$nd = count($cw);
		// check size
		if ($nd > 1558) return false;
		  // get minimum required matrix size.
		foreach ($this->symbattr[$this->shape] as $params)
			if ( (in_array($this->rows,[0,$params[0]])) && (in_array($this->cols,[0,$params[1]])) && ($params[11] >= $nd)) break;
		if ( (!in_array($this->rows,[0,$params[0]])) || (!in_array($this->cols,[0,$params[1]])) || ($params[11] < $nd)) {
			// too much data or wrong cols/rows request
			return false;
		} elseif ($params[11] > $nd) {
			// add padding
			...
			if ($params[11] > $nd) {
				...
				++$nd;
				// add remaining pads
				for ($i = $nd; $i < $params[11]; ++$i) $cw[] = $this->get253StateCodeword(129, $i);
			}
		}
		...
		$this->barcode_array = [ 'num_rows' => $params[0], 'num_cols' => $params[1], 'bcode' => $grid ];
	}
	...
	protected function getGFProduct($a, $b, $log, $alog, $gf) {
		return ((($a == 0) || ($b == 0)) ? 0 : $alog[($log[$a] + $log[$b]) % ($gf - 1)]);
	}
	...
	protected function getErrorCorrection($wd, $nb, $nd, $nc, $gf=256, $pp=301) {
		...
		$num_we = ($nb * $nc);
        // zerofill codeword vector
		
		// for each block
		for ($b = 0; $b < $nb; ++$b) {
			// create interleaved data block
			$block = [];
			for ($n = $b; $n < $num_wd; $n += $nb) $block[] = $wd[$n];
	 ...
	 * @param int $nd Number of current codewords.
	 * @return int number of data codewords in matrix
	 * @protected
	 */
	protected function getMaxDataCodewords($nd) {
		foreach ($this->symbattr[$this->shape] as $params)
			if ( ($params[0]>=$this->rows) && ($params[1]>=$this->cols) && ($params[11] >= $nd) ) return $params[11];
		return 0;
	}
